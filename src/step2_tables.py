"""Step 2: create the Case Process Configurator tables and their non-lookup columns."""
import dv
...
# ---------------------------------------------------------------- tables
TABLES = [
    ("caseprocesstemplate", "Case Process Template", "Case Process Templates",
     "A declarative blueprint describing which cases it targets and what to apply to them."),
    ("matchrule", "Process Match Rule", "Process Match Rules",
     "A single condition used to decide whether a template applies to a case."),
    ("processtask", "Process Task Template", "Process Task Templates",
     "An ordered task to generate on the case, linked to a business process stage, with an owner and a task SLA."),
    ("documentpackage", "Document Package", "Document Packages",
     "A reusable bundle of documents required for a case process."),
    ...
    ("appliedprocess", "Applied Process", "Applied Processes",
     "Runtime stamp recording which template was applied to a case and what it generated."),
    ("caserequireddocument", "Case Required Document", "Case Required Documents",
     "Runtime checklist row for a document required on a specific case."),
]

ASSIGN = [(1, "Team"), (2, "User"), (3, "Security Role"), (4, "Queue"),
          (5, "Manager of case owner"), (6, "Case owner")]
...
"caseprocesstemplate": [
    ...
    dict(name="rank", display="Rank", type="int", req="ApplicationRequired", min=1, max=1000,
         desc="Lower rank wins when several templates match the same case."),
    ...
    dict(name="bpfentityname", display="BPF Entity Name", type="str", len=100,
         desc="Logical name of the business process flow entity, e.g. phonetocaseprocess."),
    ...
    dict(name="firstresponsehours", display="First Response Target (hours)", type="dec", prec=2, max=10000),
    dict(name="resolutionhours", display="Resolution Target (hours)", type="dec", prec=2, max=10000),
    dict(name="setcasepriority", display="Set Case Priority", type="choice",
...
"matchrule": [
    ...
    dict(name="attributename", display="Case Field", type="str", len=100, req="ApplicationRequired",
         desc="Logical name of the field on the case, e.g. prioritycode."),
    dict(name="attributelabel", display="Case Field Label", type="str", len=200),
...
"processtask": [
    ...
    dict(name="slastartwhen", display="SLA Starts When", type="choice",
         options=[(1, "Case created"), (2, "Stage entered"), (3, "Predecessor completed"), (4, "Task created")],
         default=1),
...
"documentitem": [
    ...
    dict(name="responsible", display="Responsible", type="choice",
         options=[(1, "Customer"), (2, "Case owner"), (3, "Team"), (4, "Third party")], default=1),
    dict(name="duehours", display="Due In (hours)", type="dec", prec=2, max=10000),
