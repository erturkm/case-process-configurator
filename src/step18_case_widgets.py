"""Step 18: runtime widgets on the Case summary tab.

Uploads the process progress rail and the required-document checklist, then splices both into
the right-hand column of the Summary tab and adds Case Category / Customer Segment to the
Case Details Summary section.
"""
import base64
import os
import re
...
WIDGETS = [
    (f"{P}_caseprocess.html", "caseprocess.html", "Case Process Progress",
     "Visual stage rail and task timeline for the applied case process."),
    (f"{P}_casedocs.html", "casedocs.html", "Case Required Documents",
     "Checklist of required documents with drag and drop upload."),
    (f"{P}_casepanel.html", "casepanel.html", "Case Process Panel",
...
def field_rows():
    """Case Category / Customer Segment, tagged so the splice stays idempotent."""
    out = []
    for field, label in ((f"{P}_casecategory", "Case Category"),
                         (f"{P}_customersegment", "Customer Segment")):
        out.append(
...
def strip(tab):
    ...
    for field in (f"{P}_casecategory", f"{P}_customersegment"):
        tab = re.sub(rf'<row>(?:(?!</row>).)*?datafieldname="{field}"(?:(?!</row>).)*?</row>',
                     "", tab, flags=re.S)
    return tab


"""Out of the box lookups the demo does not use; they only add noise to the Summary tab."""
DROP_FIELDS = ("caseorigincode", "primarycontactid", "entitlementid", "productid")
...
def add_fields(tab):
    """Drop the two picklists at the end of the Case Details Summary section."""
    m = re.search(r'<section name="Case Details Summary".*?</section>', tab, flags=re.S)
    if not m:
...
def add_widgets(tab, ids):
    ...
    if last < 0:
        return tab, False
    secs = widget_section(SEC_PANEL, "Case process", "WebResource_cpcpanel",
