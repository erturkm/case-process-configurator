...
(step 23).  Process tasks carry their own deadline in cpc_sladue / cpc_slawarn,
...
def inject(xml, view_id):
    ...
    Every task form carries the cpc_outcome_sec section, and that is the anchor:
    ...
    anchor = xml.find('<section name="cpc_outcome_sec"')
    if anchor < 0:
        return None
...
def backfill():
    ...
    tasks = dv.get("tasks?$select=activityid,subject,cpc_sladue,cpc_slawarn,statecode,"
                   "cpc_slastatus&$filter=cpc_sladue ne null&$top=500")["value"]
    made = skipped = 0
    for t in tasks:
        ...
        body = {
            ...
            "failuretime": t["cpc_sladue"],
            "status": 4 if t["statecode"] == 1 else 0,
        }
        if t.get("cpc_slawarn"):
            body["warningtime"] = t["cpc_slawarn"]
        if t["statecode"] == 1:
            body["succeededon"] = t["cpc_sladue"]
