...
Adds:
  - cpc_assigntype option 7 = AI Agent on cpc_processtask
  - agent configuration columns on cpc_processtask (design time)
  - agent run-state columns on task (runtime)
  - cpc_agentrun, the audit table for every agent invocation
...
CONTEXT_SCOPE = [
    ...
    (2, "Case description"),
    (3, "Customer profile"),
    (4, "Customer case history"),
    (5, "Notes on the case"),
    (6, "Emails on the case"),
...
def lookup(referenced, referencing, suffix, display, desc=None):
    ...
    if attr_exists(referencing, ln):
        ...
        return
    rel = f"{P}_{referenced}_{referencing}_{suffix}".replace("cpc_cpc_", "cpc_")[:100]
...
def ensure_assigntype_option():
    """Add 7 = AI Agent to the existing cpc_assigntype picklist."""
    ...
    if 7 in existing:
        print(f"  = cpc_assigntype 7 = {existing[7]}")
        return
    ...
    print("  + cpc_assigntype 7 = AI Agent")
...
TASK = [
    ...
    # cpc_agentstate instead costs three wasted runs per task.
...
def main():
    ...
    for c in AGENTRUN:
        add(f"{P}_agentrun", c)
    lookup("incident", f"{P}_agentrun", "case", "Case")
