"""Step 60: export the solution as a zip that installs into a vanilla Customer Service org.

Why this exists
---------------
The case and sales configurators were authored in the same org, and both used to layer
their own sections onto the shared Case and Task system forms.  In that org this is
correct and wanted: a task belongs to a case or to a deal, each panel shows only in its
own context, and nothing collides.

A system form, however, is an atomic solution component.  It exports whole - there is no
way to say "export only my sections".  So a straight ExportSolution used to produce task
forms still carrying the sales sections, and a Case form still carrying Field Service's
libraries and fields.  None of that exists in a vanilla Customer Service org, so the
import failed with a missing dependency for each one, multiplied by every form shipped.

Step 57 now solves that at the source: CPC forks its own Case (CPC) and Task (CPC) forms
with foreign content already removed, and step58 drops Microsoft's forms from the
solution entirely.  The strip below is therefore a safety net rather than the fix - on a
correctly built solution it finds nothing, and saying so is itself a useful check.

What is still needed here is the manifest clean.  The platform computes the exported
<MissingDependencies> block against the org, where the shared forms do still carry the
sales sections, so stale entries survive even once the content is gone.

The external Modern SLA Timer PCF is deliberately NOT bundled - it has its own
repository and licence.  It stays a documented prerequisite, and is the only thing
besides Dynamics 365 Customer Service that this solution requires:
...
# Sections owned by the sales configurator. Anything matching is removed from the
# exported task forms, together with the controlDescription its control points at.
# Expected to match nothing: step57's fork already excludes foreign content.
FOREIGN_SECTION = re.compile(r"^spc_")
...
def audit(xml):
    ...
    hits = {}
    for tok in re.findall(r"spc_[A-Za-z0-9_.]*", xml):
...
def clean_manifest(xml, customizations):
    ...
    shared forms still carry the sales sections.  It is written into the zip as a manifest
    ...
    the two solutions name their components after each other ("Task SLA Timer Source"
    versus "Sales Task SLA Timer Source"), and it drops real platform columns whose
    names happen to appear inside longer ones.
    ...
    manifest because the case forms really do bind to it.
    """
    removed, kept = [], []
    ...
    def one(m):
        ...
        if rid:
            ...
            # construction here, because the two solutions' components are named
            # after each other.
