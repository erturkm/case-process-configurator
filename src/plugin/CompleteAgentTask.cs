...
{
    /// <summary>
    /// Custom API cpc_CompleteAgentTask. The single write-back point for an AI agent turn.
    ///
    ...
    public class CompleteAgentTask : IPlugin
    {
        ...
        // cpc_outputtarget on cpc_processtask
        ...
        // cpc_agentoutcomemode on cpc_processtask
        ...
        public void Execute(IServiceProvider sp)
        {
            ...
            Guid caseId = regarding != null && regarding.LogicalName == "incident"
            ...
            // ---------------------------------------------------------- side outputs
            if (!failed && !string.IsNullOrWhiteSpace(output) && caseId != Guid.Empty)
            {
                if (target == TargetCaseNote) CreateNote(svc, caseId, task, output);
                else if (target == TargetCaseDescription) AppendCaseDescription(svc, caseId, output);
            }

            // ---------------------------------------------------------- audit
            var runId = LogRun(svc, task, tt, caseId, taskId, attempt, runStatus, promptSent,
        ...
        private static void CreateNote(IOrganizationService svc, Guid caseId, Entity task, string output)
        {
        ...
        private static void AppendCaseDescription(IOrganizationService svc, Guid caseId, string output)
        {
            var c = svc.Retrieve("incident", caseId, new ColumnSet("description"));
            var existing = c.GetAttributeValue<string>("description") ?? "";
            var upd = new Entity("incident", caseId);
            upd["description"] = CapToAttribute(svc, "incident", "description",
                (string.IsNullOrWhiteSpace(existing) ? "" : existing + "\n\n")
                + "--- AI agent "
                + DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm", CultureInfo.InvariantCulture)
                + " UTC ---\n" + output);
        ...
        private static Guid LogRun(IOrganizationService svc, Entity task, Entity tt, Guid caseId,
                                   ...
                                   bool autoCompleted, string requestedBy)
        {
            ...
            if (caseId != Guid.Empty) run[P + "case"] = new EntityReference("incident", caseId);
            if (tt != null)
            {
