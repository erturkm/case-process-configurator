...
{
    /// <summary>
    /// Custom API cpc_DesignProcess. Reads an uploaded standard operating procedure and asks an
    /// Azure AI Foundry model to design a case process from it. Returns a blueprint in exactly the
    /// shape cpc_AuthorProcess already understands, so the existing preview and commit path is reused.
    ///
    ...
    public class DesignProcess : IPlugin
    {
        ...
        private static Cfg Config(IOrganizationService svc)
        {
            var c = new Cfg
            {
                Endpoint = EnvVar(svc, "cpc_FoundryEndpoint"),
                Deployment = EnvVar(svc, "cpc_FoundryDeployment"),
                ApiVersion = EnvVar(svc, "cpc_FoundryApiVersion"),
                TenantId = EnvVar(svc, "cpc_FoundryTenantId"),
                ClientId = EnvVar(svc, "cpc_FoundryClientId"),
                ClientSecret = EnvVar(svc, "cpc_FoundryClientSecret"),
            };
            if (string.IsNullOrWhiteSpace(c.Endpoint) || string.IsNullOrWhiteSpace(c.Deployment)
                ...
                throw new InvalidPluginExecutionException(
                    "The Process Copilot is not configured. Set the cpc_Foundry* environment variables.");
        ...
        private static string Catalog(IOrganizationService svc, ITracingService trace)
        {
            ...
            sb.AppendLine("## Business process flows available for Case");
            ...
            List(svc, sb, "## Case subjects (use for the subject match rule)", "subject", "title", null, 80, null);
        ...
        {
            var sb = new StringBuilder();
            sb.AppendLine("You are a Dynamics 365 Customer Service process designer for a bank.");
            sb.AppendLine("You convert a written standard operating procedure into a case process blueprint.");
            sb.AppendLine();
            ...
            sb.AppendLine("9. Match rules decide which cases get this process. Prefer the case subject and");
            sb.AppendLine("   the customer segment. Conditions in one group are OR'ed, groups are AND'ed.");
            sb.AppendLine();
        ...
        {
            var sb = new StringBuilder();
            sb.AppendLine("Design a case process from the standard operating procedure below.");
        ...
        private static string Foundry(Cfg cfg, string system, string user, ITracingService trace)
        {
            ...
            var body = "{\"messages\":["
                     ...
                     + "{\"name\":\"case_process\",\"strict\":true,\"schema\":" + Schema.Text + "}}}";
