...
{
    /// <summary>
    /// Custom API cpc_GetCaseView. One call that feeds both runtime widgets on the case summary
    ...
    /// </summary>
    public class GetCaseView : IPlugin
    {
        ...
        public void Execute(IServiceProvider sp)
        {
            ...
            var raw = ctx.InputParameters.Contains("CaseId") ? (string)ctx.InputParameters["CaseId"] : null;
            var caseId = ProcessRuntime.ParseGuid(raw);
            if (caseId == Guid.Empty)
                throw new InvalidPluginExecutionException("CaseId is required.");

            var incident = svc.Retrieve("incident", caseId, new ColumnSet(
                "title", "ticketnumber", "statecode",
                "stageid", P + "appliedtemplate", P + "processappliedon",
                P + "processsummary", P + "firstresponsedue", P + "firstresponsestatus",
                P + "resolutiondue", P + "resolutionstatus",
                P + "tasksopen", P + "taskstotal", P + "docsreceived", P + "docstotal"));

            var sb = new StringBuilder();
            sb.Append("{").Append(Json.Q("case")).Append(":{");
            sb.Append(Json.Q("id")).Append(":").Append(Json.Q(caseId.ToString()));
            Field(sb, "title", incident.GetAttributeValue<string>("title"));
            Field(sb, "number", incident.GetAttributeValue<string>("ticketnumber"));
            Field(sb, "summary", incident.GetAttributeValue<string>(P + "processsummary"));
            var tpl = incident.GetAttributeValue<EntityReference>(P + "appliedtemplate");
            Field(sb, "templateId", tpl == null ? null : tpl.Id.ToString());
            Field(sb, "templateName", tpl == null ? null : tpl.Name);
            DateField(sb, "appliedOn", incident.GetAttributeValue<DateTime?>(P + "processappliedon"));
            DateField(sb, "firstResponseDue", incident.GetAttributeValue<DateTime?>(P + "firstresponsedue"));
            DateField(sb, "resolutionDue", incident.GetAttributeValue<DateTime?>(P + "resolutiondue"));
            NumField(sb, "firstResponseStatus", Opt(incident, P + "firstresponsestatus"));
            NumField(sb, "resolutionStatus", Opt(incident, P + "resolutionstatus"));
            NumField(sb, "tasksOpen", incident.GetAttributeValue<int?>(P + "tasksopen"));
            NumField(sb, "tasksTotal", incident.GetAttributeValue<int?>(P + "taskstotal"));
            NumField(sb, "docsReceived", incident.GetAttributeValue<int?>(P + "docsreceived"));
            NumField(sb, "docsTotal", incident.GetAttributeValue<int?>(P + "docstotal"));
            sb.Append("}");

            AppendStages(sb, svc, caseId, tpl, incident.GetAttributeValue<Guid?>("stageid") ?? Guid.Empty);
            AppendTasks(sb, svc, caseId);
            AppendDocuments(sb, svc, caseId);
        ...
        private static void AppendStages(StringBuilder sb, IOrganizationService svc, Guid caseId,
                                         EntityReference tpl, Guid stageId)
        {
            ...
            if (tpl != null)
            {
                try
                {
                    var t = svc.Retrieve(P + "caseprocesstemplate", tpl.Id,
            ...
            current = CurrentStageName(svc, caseId, bpfEntity, stageId);
        ...
        /// <summary>
        /// The engine stamps incident.stageid when it moves the flow, and the business process flow
        /// instance mirrors it. Prefer the case column, fall back to the instance row.
        /// </summary>
        private static string CurrentStageName(IOrganizationService svc, Guid caseId,
                                               string bpfEntity, Guid stageId)
        {
            if (stageId == Guid.Empty && !string.IsNullOrEmpty(bpfEntity))
            {
                try
                {
                    ...
                    q.Criteria.AddCondition("incidentid", ConditionOperator.Equal, caseId);
        ...
        private static void AppendTasks(StringBuilder sb, IOrganizationService svc, Guid caseId)
        {
        ...
        private static void AppendDocuments(StringBuilder sb, IOrganizationService svc, Guid caseId)
        {
            ...
            var files = Attachments(svc, caseId);
        ...
        private static List<Entity> Attachments(IOrganizationService svc, Guid caseId)
        {
