...
{
    /// <summary>
    /// Custom API cpc_PrepareAgentTask. The read side of the agent turn.
    ///
    ...
    public class PrepareAgentTask : IPlugin
    {
                                    ...
                                    IPluginExecutionContext ctx, Guid taskId)
        {
            ...
            var caseId = regarding != null && regarding.LogicalName == "incident"
            ...
            var scope = CaseContext.ParseScope(tt[P + "contextscope"]);
            var context = CaseContext.Build(svc, caseId, scope);
