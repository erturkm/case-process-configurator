...
{
    /// <summary>
    /// Custom API cpc_SaveProcessGraph. Takes the whole canvas back as JSON and reconciles it against
    ...
    public class SaveProcessGraph : IPlugin
    {
        ...
        public void Execute(IServiceProvider sp)
        {
            ...
            tpl[P + "setcasepriority"] = new OptionSetValue(Json.Int(header, "setCasePriority") ?? 0);
            tpl[P + "matchlogic"] = new OptionSetValue(3);

            var fr = Dec(header, "firstResponseHours");
            if (fr.HasValue) tpl[P + "firstresponsehours"] = fr.Value;
            var rh = Dec(header, "resolutionHours");
            if (rh.HasValue) tpl[P + "resolutionhours"] = rh.Value;
            ...
            foreach (var raw2 in nodes)
            {
                ...
                e[P + "template"] = new EntityReference(P + "caseprocesstemplate", templateId);
            ...
            foreach (var raw3 in edges)
            {
                ...
                e[P + "closecase"] = Json.Bool(o, "closeCase") ?? false;
                e[P + "requirecomment"] = Json.Bool(o, "requireComment") ?? false;
                e[P + "isdefault"] = Json.Bool(o, "isDefault") ?? false;
                e[P + "setcasepriority"] = new OptionSetValue(Json.Int(o, "setCasePriority") ?? 0);
