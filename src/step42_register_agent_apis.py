...
Registers:
  cpc_BuildCaseContext   assemble grounded CRM context for a prompt
  cpc_CompleteAgentTask  write an agent turn back and advance the process
  cpc_GetAgentCatalog    published, non-system agents for the designer picker
"""
import dv
...
APIS = [
    ("BuildCaseContext", "Build Case Context", "Cpc.Plugins.BuildCaseContext", "",
     [("CaseId", 10, True), ("Scope", 10, False), ("BudgetChars", 5, False)],
     [("Context", 10), ("Length", 5)]),

    ("CompleteAgentTask", "Complete Agent Task", "Cpc.Plugins.CompleteAgentTask", "",
    ...
    ("PrepareAgentTask", "Prepare Agent Task", "Cpc.Plugins.PrepareAgentTask", "",
     [("TaskId", 10, True)],
    ...
    ("GetAgentCatalog", "Get Agent Catalog", "Cpc.Plugins.GetAgentCatalog", "",
