...
ProcessRuntime parks AI-assigned tasks as Queued and cpc_CompleteAgentTask knows how to write a
...
cpc_PrepareAgentTask and cpc_CompleteAgentTask where it can be tested. The flow only:

    trigger on a Queued AI task
      -> cpc_PrepareAgentTask   (claims the task, returns the agent + one ready message)
      -> ExecuteCopilotAsyncV2  (the long call a sandboxed plug-in cannot make)
      -> cpc_CompleteAgentTask  (parses the reply, audits, completes or parks for review)
...
FLOW_NAME = "CPC - Run AI agent task"
...
REFS = [
    ("cpc_dataverse", "CPC Dataverse", DV_API, "shared_commondataserviceforapps"),
    ("cpc_copilotstudio", "CPC Copilot Studio", MCS_API, "shared_microsoftcopilotstudio"),
    ("cpc_agentnode", "CPC Agent Node", AGN_API, "shared_agentnode"),
]
            ...
            "When_an_AI_task_is_queued": {
                ...
                "inputs": {
                    ...
                    "parameters": {
                        ...
                        # The single trigger signal. cpc_agentqueuedon is written only when a turn
                        # is genuinely requested, never by this flow, so the flow's own state
                        # writes cannot wake it. Filtering on cpc_agentstate instead costs three
                        # wasted runs per task (Running, then the completion write, then its echo).
                        "subscriptionRequest/filteringattributes": "cpc_agentqueuedon",
                    },
                    "authentication": "@parameters('$authentication')",
                },
            }
        ...
        "properties": {
            "connectionReferences": {
                "shared_commondataserviceforapps": {
                    ...
                    "connection": {"connectionReferenceLogicalName": "cpc_dataverse"},
                    "api": {"name": "shared_commondataserviceforapps"},
                },
                "shared_microsoftcopilotstudio": {
                    "runtimeSource": "embedded",
                    "connection": {"connectionReferenceLogicalName": "cpc_copilotstudio"},
                    "api": {"name": "shared_microsoftcopilotstudio"},
                },
                "shared_agentnode": {
                    "runtimeSource": "embedded",
                    "connection": {"connectionReferenceLogicalName": "cpc_agentnode"},
...
def upsert_flow(data):
    ...
    if rows:
        ...
        body.update({"category": 5, "type": 1, "primaryentity": "none",
                     "description": "Runs Case Process Configurator AI agent tasks."})
