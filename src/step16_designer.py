...
WR_DISPLAY = "Case Process Visual Designer"
HERE = os.path.dirname(os.path.abspath(__file__))
HTML_PATH = os.path.join(HERE, "webresources", "designer.html")

APP_UNIQUE = f"{P}_ProcessConfiguratorApp"
SITEMAP_NAME = f"{P}_cpcsitemap"
TEMPLATE_ENTITY = f"{P}_caseprocesstemplate"
...
DESIGNER_TAB = (
    '<tab name="cpc_designer_tab" id="{tabid}" IsUserDefined="0" locklevel="0" '
    ...
    '<sections>'
    '<section name="cpc_designer_sec" id="{secid}" IsUserDefined="0" locklevel="0" '
...
def add_tab_to_form(wid):
    ...
    for f in forms:
        xml = f["formxml"]
        if "cpc_designer_tab" in xml:
            xml = re.sub(r'<tab name="cpc_designer_tab".*?</tab>', "", xml, flags=re.S)
        if "<tabs>" not in xml:
...
def add_sitemap_item(app_id):
    ...
    if not sm:
        ...
        return None
    xml = sm["sitemapxml"]
    if "cpc_sub_designer" in xml:
        xml = re.sub(r'<SubArea Id="cpc_sub_designer".*?</SubArea>', "", xml, flags=re.S)
    sub = (
        '<SubArea Id="cpc_sub_designer" '
