"""Step 58: point the app at the CPC forms, and let go of Microsoft's.

Two halves, and they belong together:

1. Register the forms step57 created as app components. An app that lists no forms for an
   entity offers every form on that entity, and which one opens is then decided by form
   order and security roles -- so the CPC widgets were reachable only by luck. Naming the
   forms explicitly makes the CPC app open them every time.

2. Drop Microsoft's Case and Task forms from the solution. This is the change that stops
   CPC overwriting other people's work: a system form is an atomic component, so while
   those forms were in the solution CPC shipped a whole copy of each -- six in total -- and
   stamped it over whatever the target org had.

Removing a form from the solution does not delete it or touch its contents. The sections
CPC added to them in this org stay exactly where they are, as does anything SPC put there.
"""
import dv

P = dv.PREFIX

APP_UNIQUE = f"{P}_ProcessConfiguratorApp"
FORM_COMPONENT = 60

# Microsoft's forms, which CPC must no longer claim. Named rather than hardcoded by guid so
# this reads as an intent and still works in an org where the guids differ.
RELEASE = [
    ("incident", "Case"),
    ("incident", "Case for Interactive experience"),
    ("incident", "Case for Multisession experience"),
...
# The forms CPC owns, created by step57.
OWNED = [
    ("incident", f"Case ({P.upper()})"),
    ("task", f"Task ({P.upper()})"),
]
...
def add_to_app():
    ...
    for entity, name in OWNED:
        ...
        if not fid:
            print(f"  ! missing {entity} form '{name}' - run step57 first")
            continue
...
def main():
    print("Registering CPC forms with the app")
    add_to_app()
    ...
    for fid in sorted(solution_components(dv.SOLUTION)):
        ...
        if f["objecttypecode"] in ("incident", "task"):
