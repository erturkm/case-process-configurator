"""Step 48 (Phase 8): end to end test of cpc_DesignProcess from an uploaded .docx.

Uploads the procedure as an annotation, asks the Copilot to design a process from it,
...
DOC = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                   "demo", "RAKBANK_OPS-CRD-014_Card_Dispute_Procedure.docx")
...
def main():
    ...
    try:
        t0 = time.time()
        prep = dv.post("cpc_DesignProcess", {
            "AnnotationId": nid,
            "Instructions": "This is for RAKBANK retail cards in the UAE.",
            "Mode": "prepare",
        })
        ...
        print(f"RANK  : {d.get('rank')}   priority={d.get('setCasePriority')}   publish={d.get('publish')}")
        print(f"SLA   : first={d.get('firstResponseHours')}h  resolution={d.get('resolutionHours')}h")
        ...
        print("\n" + "=" * 78)
        print("now rendering through cpc_AuthorProcess preview ...")
        pv = dv.post("cpc_AuthorProcess", {"ProcessJson": r["ProcessJson"], "Mode": "preview"})
